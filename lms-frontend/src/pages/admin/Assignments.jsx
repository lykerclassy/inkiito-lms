...
export default function AdminAssignments() {
    ...
    if (isLoading) return <div className="p-8 text-gray-500 font-medium">Loading assignments...</div>;
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden max-h-[90vh] flex flex-col">
                        ...
                        <div className="overflow-y-auto flex-1 p-6 space-y-6 bg-gray-50">
                            {activeSubmissions.length === 0 ? (
                                <div className="text-center text-gray-500 py-10">No students have submitted yet.</div>
                            ) : (
                                activeSubmissions.map(submission => (
                                    <Card key={submission.id} className="border border-gray-200">
                                        <div className="flex justify-between items-start mb-4 gap-4">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className={`text-xs font-bold px-2 py-0.5 rounded uppercase tracking-wider ${submission.status === 'graded' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                        {submission.status}
                                                    </span>
                                        ...
                                        <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
                                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Teacher's Feedback (Private to student)</label>
                                            <textarea 
                                                rows="3" 
                                                placeholder="Type personalized feedback here..." 
                                                className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-green-500 text-sm bg-white"
